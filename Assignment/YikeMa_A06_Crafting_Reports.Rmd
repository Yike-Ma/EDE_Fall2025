...
```{r project.setup, message=FALSE, error=FALSE, echo=TRUE, warning=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r, echo=FALSE, fig.cap="Monthly Variation of Ozone Concentration in 2018",fig.align='left', fig.height=3}
#distribution of Ozone levels by month using
library(ggplot2)
#2018 plot
ozone2018 <- epa_data %>% 
  filter(Year == 2018) %>% 
  na.omit() %>% 
  ggplot(mapping = aes(x = Month, y = Ozone, group = Month))+
  geom_boxplot()+
  scale_x_continuous( #change month from int to name
...
```{r, echo=FALSE, fig.cap="Monthly Variation of Ozone Concentration in 2019", fig.align='left', fig.height=3}
#2019 plot
ozone2019 <- epa_data %>% 
  filter(Year == 2019) %>% 
  na.omit() %>% 
...
## Git Repository
Here is the [GitHub link](https://github.com/Yike-Ma/EDE_Fall2025.git) for my repository.

<https://github.com/Yike-Ma/EDE_Fall2025.git>
