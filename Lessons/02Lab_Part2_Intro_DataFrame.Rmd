...
car_speed #pull a vector from dataframe
```
...
#Option 1
df <- cbind(df_car_speed,car_dist) #c for column #similarly rows could be added using rbind() 
class(df)
df

#Or Option 2 - transform into a data frame before binding, as long as they have same length
...
#you could also use this notation to delete columns (delete columns with negative)
df <- df[,-2]
df
```
