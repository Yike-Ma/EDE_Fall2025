...
PMplot <- 
  ...
  geom_point()
print(PMplot)
# shape decides the shape of the dot
# Separate plot with facets
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year))) +
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) # 3 rows, plotted by site name
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #year as column, so shape doesn't need to be defined here
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #reverse the y-axis because of oceanology standards
print(tempvsdepth)
...
NvsP <-
  ...
  geom_abline(aes(slope = 16, intercept = 0)) # a specified line
print(NvsP)
...
Nutrientplot4 <-
  ...
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #"dodge" -> make sure the dots do not overlaying
print(Nutrientplot4)
...
# Ridgeline plot, y is frequency but categorized
Nutrientplot6 <-
