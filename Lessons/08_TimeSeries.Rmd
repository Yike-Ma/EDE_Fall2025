...
stl (seasonal trend linear model): use univariate time series as x
...
# Visualize the decomposed series. bars on the right part of the plot means they scale of how they are zoomed in.
...
EnoDischarge.late_Components <- mutate(EnoDischarge.late_Components,
        ...
        Date = EnoDischarge.late$datetime)#adding two more colunm
...
# na.approx: Generic functions for replacing each NA with interpolated values.
wind_data_clean <- 
  wind_data %>% 
...
plot(wind_data_decomp) #it's seasonal data
...
# Inspect results
wind_data_trend1
summary(wind_data_trend1) #p value<0.5 ->reject the null hypothesis
