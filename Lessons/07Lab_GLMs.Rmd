...
ozone2 <-
  ggplot(
    EPAair,aes(x=as.factor(Year),y= Ozone))+
      geom_boxplot()+
...
Airregression <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(Airregression)
...
EPAairCorr <- cor(EPA.subset)
corrplot(EPAairCorr, method = "ellipse")


Ex4 <- lm (data = EPA.subset, PM2.5 ~ mean_Lat + mean_Lng + Ozone + Month + Year)
summary(Ex4)
