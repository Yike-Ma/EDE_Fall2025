...
output: html_document #pdf document
runtime: shiny
editor_options: 
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = !!sym(the_parameter),  #it doesn't work if I change the y = xxx to the value I assigned
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) + #alpha is transparency
  scale_x_date(
...
sliderInput(
  inputId = 'year_slider',
  label = 'Select a start year and end year',
...
renderPlot(
  plot_it(
    input$nutrient,
    input$year_slider[1],#indicates the first value from the widgets
    input$year_slider[2]
  )
)
...
renderText(
  paste("you selected ", input$nutrient, "from", input$year_slider[1], "to", input$year_slider[2])
)
