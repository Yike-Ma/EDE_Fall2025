...
NTL_data <- read.csv(file = here("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"),
                     stringsAsFactors = TRUE)

# 2.
library(tidyverse);library(here)
EPAairs <- dir(here('Data','Raw'),pattern = ("^EPAair_O3.*csv"))
...
files <- dir(folder, pattern = "EPAair_O3*.*", full.names = TRUE) #full.names = true, means to create a complete path
EPAdata <- files %>%
  map(read_csv) %>%
  reduce(rbind)  #binding data by rows. alternative: bind_rows()
...
class(str_juneteenth) #character
class(date_juneteenth) #date
```
### The century issue
