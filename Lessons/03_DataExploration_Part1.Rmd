...
library(ggplot2)
library(here)
# May return as: "The following objects are masked from ‘package:stats’:
                  #filter, lag"
#This is because other packages (eg. stats) have same functions (filter, log), in this case, to make sure we are using the function from dplyr, we type "dplyr::filter()" in the console
...
# Relative file path (friendly for users regardless of machine) and save the data to variable
USGS.flow.data <- read.csv(
  file = here("./Data/Raw/USGS_Site02085000_Flow_Raw.csv"),
  stringsAsFactors = TRUE)   
# factors are characters, but in R they are treated as categorical value, rather than raw strings.
...
```{r Exploring data}
#View(USGS.flow.data) # this should be commented out before knitting
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data)
colnames(USGS.flow.data) #names() works as well
...
mean(USGS.flow.data$discharge.mean) #return as "NA"
